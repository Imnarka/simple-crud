...
func (h *Handler) GetTaskByIDHandler(w http.ResponseWriter, r *http.Request) {
	...
	logger.Info("Задача успешно получена: ", logrus.Fields{"tasks": response})
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(response)
}
...
func (h *Handler) UpdateTaskHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		msg := "Ошибка обновления задачи"
		logger.Error(msg, logrus.Fields{"error": err, "task_id": taskId})
		http.Error(w, msg, http.StatusInternalServerError)
		return
	}
	response := dto.TaskResponse{
	...
	logger.Info("Задача успешно обновлена", logrus.Fields{"task_id": taskId})
	w.Header().Set("Content-Type", "application/json")
